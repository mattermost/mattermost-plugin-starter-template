...
import (
	...
	"github.com/mattermost/mattermost-plugin-starter-template/build/manifest/vargen"
...
func applyManifest(manifest *model.Manifest) error {
	if manifest.HasServer() {
		// generate Go source code for manifest.
		code := vargen.Generate("manifest", manifest)
		// write generated code to file by using Go file template.
		if err := ioutil.WriteFile(
			"server/manifest.go",
			[]byte(fmt.Sprintf(pluginIDGoFileTemplate, code)),
			0644,
		); err != nil {
	...
	if manifest.HasWebapp() {
		// generate JSON representation of manifest. JSON is very similar and compatible
		// with JS's object literals. so, what we do here is actually JS code generation.
		...
		if err := ioutil.WriteFile(
			...
			[]byte(fmt.Sprintf(pluginIDJSFileTemplate, string(data))),
			0644,
		); err != nil {
