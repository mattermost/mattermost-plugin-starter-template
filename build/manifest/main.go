...
const autogeneratedFileComment = "// This file is automatically generated. Do not modify it manually."

const pluginIDGoFileTemplate = `%s
...
const pluginIDJSFileTemplate = `%s
export const id = '%s';
...
func applyManifest(manifest *model.Manifest) error {
	if manifest.HasServer() {
		if err := ioutil.WriteFile(
			"server/manifest.go",
			[]byte(fmt.Sprintf(pluginIDGoFileTemplate, autogeneratedFileComment, manifest.Id, manifest.Version)),
			0644,
		); err != nil {
	...
	if manifest.HasWebapp() {
		if err := ioutil.WriteFile(
			...
			[]byte(fmt.Sprintf(pluginIDJSFileTemplate, autogeneratedFileComment, manifest.Id, manifest.Version)),
			0644,
		); err != nil {
